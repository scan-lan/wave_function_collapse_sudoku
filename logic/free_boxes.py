...
from logic.wave_function import collapse, constrain_all, get_collapsed_value, propagate
...
def fill_free_boxes(
    ...
    for free_coords in free_cell_coords:
        if len(values) == 0:
            values = get_random_values(box_size)
        collapse(coef_matrix, free_coords, weights, collapsed, value=values.pop())
        if visualise:
            print_coef_matrix(
                coef_matrix, box_dimensions, sleep=(2 / speed), new_collapse=free_coords
            )
    for coords in [
        (y, x)
        ...
        y, x = coords
        constrain_all(coef_matrix, box_dimensions, coords)
        if len(coef_matrix[y][x]) == 1:
            collapse(
                ...
                weights,
                collapsed,
                value=get_collapsed_value(coef_matrix[y][x]),
            )
